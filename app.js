    ...
    activeWorkout.exercises.forEach((ex, exIdx) => {
        ...
        html += `</div>
            <div class="active-ex-actions">
                ...
                <button class="btn btn-secondary btn-sm ${ex.supersetWith !== undefined ? 'superset-active' : ''}" onclick="toggleSuperset(${exIdx})" title="Superset with next exercise">${ex.supersetWith !== undefined ? 'Superset \u2713' : 'Superset'}</button>
                <button class="btn btn-secondary btn-sm" onclick="removeExerciseFromActive(${exIdx})" style="color:#ff5555">&times; Remove</button>
            </div>
        </div>`;
        // Visual superset connector
        if (ex.supersetWith !== undefined) {
            html += `<div class="superset-connector">Superset \u2014 no rest between</div>`;
        }
    });
...
function removeExerciseFromActive(exIdx) {
    ...
    // Clean up any superset references
    activeWorkout.exercises.forEach((ex, i) => {
        if (ex.supersetWith !== undefined && ex.supersetWith >= activeWorkout.exercises.length) {
...
function completeSet(exIdx, sIdx) {
    ...
    // Skip rest timer if exercise is in a superset (move to next exercise instead)
    if (ex.supersetWith !== undefined) {
        showToast('Superset \u2014 move to next exercise!', 'info');
    } else {
...
// 1RM Calculator (Epley + Brzycki + Lombardi average)
function calculate1RM() {
    ...
    if (!weight || !reps || reps < 1) return showToast('Enter weight and reps', 'warn');
    const unit = wu();
    const el = document.getElementById('orm-result');
    el.classList.remove('hidden');

    if (reps === 1) {
        el.innerHTML = `<p><strong>Your 1RM:</strong> ${lbsToDisplay(weight)} ${unit} (that IS your 1RM)</p>`;
        return;
    }

    // Three formulas for accuracy
    const epley = weight * (1 + reps / 30);
    const brzycki = weight * (36 / (37 - reps));
    const lombardi = weight * Math.pow(reps, 0.1);
    const avg = Math.round((epley + brzycki + lombardi) / 3);

    let html = `<h3 style="margin:12px 0 8px">Estimated 1RM: <strong>${lbsToDisplay(avg)} ${unit}</strong></h3>`;
    html += `<table class="plan-table"><tr><th>%1RM</th><th>Weight</th><th>Use For</th></tr>`;
    const pcts = [
        [100, 'Max single'], [95, '2 reps'], [90, '3-4 reps'],
        [85, '5 reps (strength)'], [80, '6-8 reps'], [75, '8-10 reps'],
        [70, '10-12 reps (hypertrophy)'], [65, '12-15 reps'], [60, '15-20 reps (endurance)']
    ];
    pcts.forEach(([pct, use]) => {
        html += `<tr><td>${pct}%</td><td><strong>${lbsToDisplay(Math.round(avg * pct / 100 / 5) * 5)} ${unit}</strong></td><td>${use}</td></tr>`;
    });
    html += `</table>`;
    html += `<p style="color:#888;font-size:12px;margin-top:8px">Average of Epley (${lbsToDisplay(Math.round(epley))}), Brzycki (${lbsToDisplay(Math.round(brzycki))}), Lombardi (${lbsToDisplay(Math.round(lombardi))})</p>`;
    el.innerHTML = html;
}
...
function calculatePlates() {
    ...
    if (!target || target <= bar) {
        el.innerHTML = `<p>Target must be more than the bar weight (${lbsToDisplay(bar)} ${unit}).</p>`;
        return;
    }

    const perSide = (target - bar) / 2;
    // Standard plates in lbs (works for both since we display in user's unit)
    const plates = bar >= 20 && bar <= 25
    ...
    let html = `<h3 style="margin:12px 0 8px">Loading: ${lbsToDisplay(target)} ${unit}</h3>`;
    html += `<p>Bar: ${lbsToDisplay(bar)} ${unit} + ${lbsToDisplay(perSide)} ${unit} per side</p>`;

    if (remaining > 0.1) {
        html += `<p style="color:#ffa500">Can't make exactly ${lbsToDisplay(target)} ${unit} with standard plates. Closest: ${lbsToDisplay(target - remaining * 2)} ${unit}.</p>`;
    }
    ...
    if (loaded.length > 0) {
        ...
        loaded.forEach(p => {
            const h = Math.max(24, Math.min(80, p * (bar >= 20 ? 2.5 : 1.2)));
            const label = bar >= 20 && bar <= 25 ? p : p;
            html += `<div class="plate-disc" style="height:${h}px"><span>${lbsToDisplay(label)}</span></div>`;
        });
        ...
        // Text summary
        const counts = {};
        loaded.forEach(p => { counts[p] = (counts[p] || 0) + 1; });
        html += `<p style="margin-top:8px"><strong>Each side:</strong> `;
        html += Object.entries(counts).map(([p, c]) => `${c}×${lbsToDisplay(parseFloat(p))}`).join(' + ');
        html += `</p>`;
    }
...
let _amrapInterval = null;
function startAmrapTimer() {
    const min = parseInt(document.getElementById('amrap-min').value) || 0;
    const sec = parseInt(document.getElementById('amrap-sec').value) || 0;
    const total = min * 60 + sec;
    if (total <= 0) return showToast('Set a time', 'warn');

    if (_amrapInterval) clearInterval(_amrapInterval);
    const endsAt = Date.now() + total * 1000;
    const display = document.getElementById('amrap-display');
    const timeEl = document.getElementById('amrap-time');
    display.classList.remove('hidden');
    document.getElementById('amrap-start-btn').classList.add('hidden');
    ...
    const update = () => {
        ...
        if (remaining === 0) {
            ...
            setTimeout(() => {
                ...
                document.getElementById('amrap-start-btn').classList.remove('hidden');
            }, 3000);
        }
    };
    update();
    _amrapInterval = setInterval(update, 250);
}

function stopAmrapTimer() {
    if (_amrapInterval) clearInterval(_amrapInterval);
    _amrapInterval = null;
    document.getElementById('amrap-display').classList.add('hidden');
    document.getElementById('amrap-start-btn').classList.remove('hidden');
}
...
function exportWorkouts() {
    const workouts = DB.get('workouts', []);
    if (!workouts.length) return showToast('No workouts to export', 'warn');
    const unit = wu();
    let csv = 'Date,Exercise,Set,Weight (' + unit + '),Reps,Volume\n';
    workouts.forEach(w => {
        w.sets.forEach((s, i) => {
            csv += `${w.date},${w.name.replace(/,/g, ';')},${i + 1},${lbsToDisplay(s.weight)},${s.reps},${lbsToDisplay(s.weight * s.reps)}\n`;
        });
    });
    _downloadCSV(`ironfaith-workouts-${today()}.csv`, csv);
}

function exportNutrition() {
    const meals = DB.get('meals', []);
    if (!meals.length) return showToast('No meals to export', 'warn');
    let csv = 'Date,Food,Calories,Protein (g),Carbs (g),Fat (g)\n';
    meals.forEach(m => {
        csv += `${m.date},${(m.name || '').replace(/,/g, ';')},${m.calories},${m.protein},${m.carbs},${m.fat}\n`;
    });
    _downloadCSV(`ironfaith-nutrition-${today()}.csv`, csv);
}

function exportBodyWeight() {
    const weights = DB.get('weights', []);
    if (!weights.length) return showToast('No weights to export', 'warn');
    const unit = wu();
    let csv = 'Date,Weight (' + unit + ')\n';
    weights.forEach(w => {
        csv += `${w.date},${lbsToDisplay(w.weight)}\n`;
    });
    _downloadCSV(`ironfaith-weight-${today()}.csv`, csv);
}

function exportMeasurements() {
    const measurements = DB.get('measurements', []);
    if (!measurements.length) return showToast('No measurements to export', 'warn');
    let csv = 'Date';
    const sites = ['neck','shoulders','chest','leftBicep','rightBicep','waist','hips','leftThigh','rightThigh','leftCalf','rightCalf','leftForearm','rightForearm'];
    sites.forEach(s => csv += ',' + s);
    csv += '\n';
    measurements.forEach(m => {
        csv += m.date;
        sites.forEach(s => csv += ',' + (m[s] || ''));
        csv += '\n';
    });
    _downloadCSV(`ironfaith-measurements-${today()}.csv`, csv);
}
...
function exportAllData() {
    const data = {
        ...
        meals: DB.get('meals', []),
        weights: DB.get('weights', []),
        measurements: DB.get('measurements', []),
        prs: DB.get('prs', []),
        achievements: DB.get('achievements', []),
        myRoutines: DB.get('myRoutines', []),
...
function toggleSuperset(exIdx) {
    if (!activeWorkout || !activeWorkout.exercises[exIdx]) return;
    const ex = activeWorkout.exercises[exIdx];
    // Toggle superset with next exercise
    if (ex.supersetWith !== undefined) {
        delete ex.supersetWith;
    } else if (exIdx < activeWorkout.exercises.length - 1) {
        ex.supersetWith = exIdx + 1;
    } else {
        showToast('Need another exercise below to superset with', 'warn');
        return;
    }
    renderActiveWorkout();
    persistActiveWorkout();

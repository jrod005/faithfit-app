...
const SYNCED_KEYS = new Set([
    ...
    'bibleVersion', 'savedTemplates', 'coachHistory',
    ...
    set(key, value) {
        try {
            ...
            if (e.name === 'QuotaExceededError') {
                showToast('Storage full — try exporting and clearing old data', 'warn');
            }
            return;
        }
        ...
        .catch(() => {
            ...
            resultDiv.innerHTML = `
                <div class="barcode-product">
                    ...
                    <button class="btn btn-primary btn-full" onclick="onBarcodeScanned('${barcode}')">Retry Lookup</button>
                    <button class="btn btn-secondary btn-full" style="margin-top:6px" onclick="closeBarcodeScanner()">Enter Manually</button>
                </div>`;
            const retryOnReconnect = () => {
                window.removeEventListener('online', retryOnReconnect);
                if (document.getElementById('barcode-modal') && !document.getElementById('barcode-modal').classList.contains('hidden')) {
...
function confirmDialog(message, opts) {
    return new Promise(resolve => {
        ...
        overlay.setAttribute('role', 'dialog');
        overlay.setAttribute('aria-modal', 'true');
        overlay.setAttribute('aria-label', o.okText || 'Confirm');
        overlay.innerHTML = `
